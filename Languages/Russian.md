...
<!--- а вообще `♫ Лучше купи Vega'у, 56-ую Vega'у ♫ :D` --->
...
> 
> На браузерах, кроме Фаерфокса и его форках (Zen Browser и Librewolf например) есть проблема, что через браузер открыть файлы для карт и скинов не всегда получается, так что открывать их надо через файловый менеджер.
>
...
# 5 Дополнения
## 5.1 Discord
Если вы хотите использовать Дискорд для трансляций со звуком, используйте ~~[Vesktop](https://github.com/Vencord/Vesktop)~~ клиент, установленный из официальных репозиториев:
```bash
